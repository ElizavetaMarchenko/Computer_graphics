...
bool useGPUCulling = true; // по умолчанию
...
struct AABBData {
    DirectX::XMUINT4 numShapes; // используем только первый UINT
...
bool IsBoxInside(in FrustumBuffer frustum, in float3 bbMin, in float3 bbMax) {
    for (int i = 0; i < 6; i++) {
        const float3 norm = frustum.planes[i].xyz;
        float4 p = float4(
        ...
        float s = dot(p, frustum.planes[i]);
        if (s < 0.0f)
            return false;
    }
    return true;
...
StructuredBuffer<uint> visibleInstances : register(t2);
...
struct VSInput
{
    ...
    nointerpolation uint originalId : ORIGINAL_ID;
};
...
VSOutput vs(VSInput vertex)
{
    ...
    uint actualId = vertex.originalId;
...
//#pragma comment(lib, "d3dcompiler.lib")
//
//static bool useGPUCulling = true; // или false для CPU culling
//
//struct TextureVertex {
//    float x, y, z; // Позиция вершины
//    float u, v; // Текстурные координаты
...
//        // Инициализируем структуру в начале
//        indirectArgs[0] = 36; // IndexCountPerInstance
//        indirectArgs[1] = 0;  // InstanceCount (будет увеличиваться)
...
//    
//    uint actualId = vertex.originalId;
//    //uint actualId = visibleInstances[vertex.instanceId];
