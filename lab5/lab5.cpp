...
void Render(ID3D11DeviceContext* pDeviceContext, ID3D11RenderTargetView* pRenderTargetView, ID3D11DepthStencilView* pDepthStencilView,
    ...
    ID3D11BlendState* pTransBlendState, ID3D11DepthStencilState* pNoWriteDepthStencilState, const DirectX::XMFLOAT3& cameraPosition)
{
    ...
    pDeviceContext->VSSetConstantBuffers(0, 1, &pGeomBuffer2); // Используем второй буфер
    pDeviceContext->DrawIndexed(36, 0, 0);
    ...
    UINT squareStrides[] = { sizeof(TextureVertex) };
    ...
    SquareInfo squares[] = {
        { GetSquareCenter(0), DirectX::XMFLOAT4(1.0f, 0.0f, 0.0f, 0.5f), 0.0f, 0 }, // Красный квадрат (индексы 0-5)
        { GetSquareCenter(4), DirectX::XMFLOAT4(1.0f, 1.0f, 0.0f, 0.5f), 0.0f, 6 }  // Желтый квадрат (индексы 6-11)
    };
    ...
    for (const auto& square : squares) {
        ...
        pDeviceContext->PSSetConstantBuffers(1, 1, &pColorBuffer);
...
void UpdateRotation(double deltaTime, ID3D11DeviceContext* pDeviceContext, ID3D11Buffer* pGeomBuffer, ID3D11Buffer* pGeomBuffer2, ID3D11Buffer* pSphereGeomBuffer,
    ID3D11Buffer* pSphereSceneBuffer, ID3D11Buffer* pSquareGeomBuffer, double& angle_y, double& angle_xz, double& cameraRadius, DirectX::XMFLOAT3& cameraPosition) {
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    HRESULT hr = pDevice->CreateBuffer(&squareGeomDesc, nullptr, &pSquareGeomBuffer);
    if (FAILED(hr)) {
        return -1;
    }
    ...
    while (msg.message != WM_QUIT) {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            // Обновление вращения
            UpdateRotation(elapsed.count(), pDeviceContext, pGeomBuffer, pGeomBuffer2, pSphereGeomBuffer, pSphereSceneBuffer, pSquareGeomBuffer, angle_y, angle_xz, cameraRadius, cameraPosition);
            ...
            Render(pDeviceContext, pRenderTargetView, pDepthStencilView, pIndexBuffer, pVertexBuffer, pInputLayout, pVertexShader, pPixelShader, pGeomBuffer, pGeomBuffer2, pSampler, pTextureView,
                ...
                pSquareVertexBuffer, pSquareIndexBuffer, pSquareInputLayout, pSquareVertexShader, pSquarePixelShader, pSquareGeomBuffer, pColorBuffer, pNoCullRasterizerState, pTransBlendState, pNoWriteDepthStencilState, cameraPosition);
